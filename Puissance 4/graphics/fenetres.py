...
class Saisie(Entry):
        ...
        def __init__(self,f,**parametres) :
                ...
                Entry.__init__(self,f,bg=c,width=l,font = (sp,tp))
                self.insert(0,td)
                # affichage dans la grille
                ligne, colonne = p[0][0],p[0][1]
                nb_lign , nb_col = p[1][0]-p[0][0]+1 , p[1][1]-p[0][1]+1
...
class Creer_photo(PhotoImage):
        """ Classe permettant de créer une image affichable
        sur une zone graphique """
        def __init__(self,fichier_GIF):
                """constructeur prenant en paramètre un fichier image de type GIF """
                PhotoImage.__init__(self,file = fichier_GIF)
...
class Graphique(Canvas):
        ...
        def aff_image(self, photo, p):
                """ Dessine une image à partir de photo de type Photo """
                im = self.create_image(p.x, p.y, image=photo)
                self.grid()
                return im
        ...
        def deplace(self,objet,dx,dy):
                """deplave un objet de la zone graphique de dx et dy"""
                self.move(objet,dx,dy)
